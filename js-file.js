...
function updateReadState(readState, id) {
  const index = library.findIndex(book => book.id == id)
  if(index !== -1) {
    library[index].readState = readState;
  }
}
...
document.getElementById("contentGrid").addEventListener('click', function(event) {
  const icoSpan = event.target.closest('.ico')
  if (icoSpan) {
    const card = icoSpan.closest('.card');
    const readState = card.querySelector('.readState')
    console.log(card.id, icoSpan.id);
    if (icoSpan.id === "deleteTitle") {
      deleteFromLibrary(card.id);
    } else if(icoSpan.id === "markUnread"){
        updateReadState("Unread", card.id);
        readState.textContent = "Read State: Unread";
    } else if (icoSpan.id === "markReading") {
        updateReadState("Reading", card.id)
        readState.textContent = "Read State: Reading";
    } else if (icoSpan.id === "markFinished") {
        updateReadState("Finished", card.id)
        readState.textContent = "Read State: Finshed";
    }
  };
})
